...
func TestDrawLineBG(t *testing.T) {
	...
	copy(ppu.vRAM.tileData[tileAddr/16][:], tileData[:])
	...
	ppu.vRAM.tileMaps[0x400+32+25] = tileId
	ppu.vRAM.tileMaps[0x400+32+13] = tileId
...
func TestDrawLineWindow(t *testing.T) {
	...
	copy(ppu.vRAM.tileData[tileBGAddr/16][:], tileBGData[:])
	...
	copy(ppu.vRAM.tileData[tileWindowAddr/16][:], tileWindowData[:])

	// Line 2 should take the first 10 pixels (WX = 17) of the BG and the others from the window
	ppu.vRAM.tileMaps[0x400] = tileBGId
	ppu.vRAM.tileMaps[0x400+1] = tileBGId
	ppu.vRAM.tileMaps[0] = tileWindowId
